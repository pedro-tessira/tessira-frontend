      ...
      <div className="space-y-6">
                        ...
                        {risk.recommendations.slice(0, 2).map((rec) => (
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              else if (rec.initiativeId) navigate(`/app/work/initiatives/${rec.initiativeId}`);
                            }}
                          >
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="space-y-3">
            ...
            {[
              { to: "/app/horizon", icon: BarChart3, label: "Horizon", desc: "Timeline, capacity & planning" },
              { to: "/app/signals", icon: Activity, label: "Signals", desc: "Risk analysis & resilience" },
              { to: "/app/skills", icon: Target, label: "Skills", desc: "Coverage, gaps & ownership" },
              { to: "/app/work/initiatives", icon: Rocket, label: "Initiatives", desc: "Delivery streams & staffing" },
            ].map((item) => (
              <Link
                key={item.to}
                to={item.to}
                className="flex items-center gap-3 rounded-lg border border-border/50 bg-card p-3 hover:border-primary/30 transition-colors group"
              >
                ...
                <ArrowUpRight size={12} className="text-muted-foreground/40 shrink-0" />
              </Link>
            ))}
          </div>
        </div>
